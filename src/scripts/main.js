...
document.addEventListener('DOMContentLoaded', () => {
  ...
  detailButtons.forEach((button, index) => {
    ...
    button.addEventListener('mousedown', function (event) {}, true);

    button.addEventListener('touchstart', function (event) {}, true);
  });
...
function getFormSubmitUrl() {
  ...
  if (
    hostname.includes('hour-of-ai-landing-junyi.web.app') ||
...
institutionTypeRadios.forEach((radio) => {
  radio.addEventListener('change', (e) => {
    if (e.target.value === '學校') {
      ...
      schoolFields
        .querySelectorAll('input[name="schoolName"], input[name="gradeLevel"]')
        .forEach((input) => {
...
deliveryFormatCheckboxes.forEach((checkbox) => {
  checkbox.addEventListener('change', () => {
    const onlineChecked = Array.from(deliveryFormatCheckboxes).some(
      (cb) => cb.value === '線上' && cb.checked
    );
...
// Events configuration with fallback data
const upcomingEventsConfig = {
  ...
  events: [], // Will be populated from API or fallback
  fallbackEvents: [
    {
      id: 'fallback-1',
...
/**
 * Process events data: validate, filter active, filter expired, sort
 */
...
function getEventsApiUrl() {
  ...
  if (!CONFIG.isWordPressConfigValid()) {
...
function getCachedEvents() {
  try {
    ...
    if (age > upcomingEventsConfig.CACHE_DURATION) return null;
...
async function fetchUpcomingEvents(forceRefresh = false) {
  ...
  try {
    ...
    if (events.length > 0) {
      ...
      console.log('📋 Events: API returned no active events, using fallback');
      return processEventsData(upcomingEventsConfig.fallbackEvents);
    }
  } catch (error) {
    console.warn('⚠️ Events API fetch failed:', error.message);
    // Try stale cache
    try {
      const stale = localStorage.getItem(upcomingEventsConfig.CACHE_KEY);
      if (stale) {
        console.log('📋 Events: Using stale cache');
...
/**
 * Get total pages (uses current loaded events)
 */
function getTotalPages() {
  const visibleCards = getVisibleCardsCount();
  return Math.ceil(upcomingEventsConfig.events.length / visibleCards);
}
...
/**
 * Hide or show the upcoming events section based on events availability
 */
function toggleEventsSection(hasEvents) {
...
async function initEventsCarousel() {
  ...
  // Fetch events from API (with caching and fallback)
  const events = await fetchUpcomingEvents();
  upcomingEventsConfig.events = events;

  // Hide section if no events
  if (events.length === 0) {
    console.log('📋 Events: No active events, hiding carousel section');
    toggleEventsSection(false);
    return;
  }

  toggleEventsSection(true);
...
const globalRankConfig = {
  CSV_URL:
    'https://docs.google.com/spreadsheets/d/1QDTmNNP3i6Nfhg6y7qp5V_cSL6ciNsMyF3bEjyKXTsY/export?format=csv',
...
function analyzeTaiwanGlobalRank(data) {
  ...
  const sorted = data.filter((item) => item.count > 0).sort((a, b) => b.count - a.count);

  // Find Taiwan
  const taiwanIndex = sorted.findIndex((item) => item.country === globalRankConfig.TARGET_COUNTRY);
  ...
  const nearbyCountries = [
    ...sorted.slice(Math.max(0, taiwanIndex - contextRange), taiwanIndex).map((country, idx) => ({
      ...country,
    ...
    ...sorted.slice(taiwanIndex + 1, taiwanIndex + contextRange + 1).map((country, idx) => ({
      ...country,
      rank: rank + idx + 1,
      position: 'below',
    })),
...
function updateGlobalRankUI(rankData, localStats) {
  ...
  animateCounter(document.getElementById('globalRankNumber'), 0, rankData.globalRank, 1000);
  ...
  document.getElementById('localEventCount').textContent = formatNumberResponsively(
    localStats.totalEvents || 0
  );
  document.getElementById('globalEventCount').textContent = formatNumberResponsively(
    rankData.taiwanCount
  );

  // Update nearby countries list
  const listContainer = document.getElementById('nearbyCountriesList');
  listContainer.innerHTML = rankData.nearbyCountries
    .map(
      (country) => `
...
window.addEventListener('resize', () => {
  ...
  resizeTimer = setTimeout(() => {
    ...
    if (totalParticipantsEl && totalParticipantsEl.dataset.value) {
      totalParticipantsEl.textContent = formatNumberResponsively(
        parseInt(totalParticipantsEl.dataset.value)
      );
    }
    if (totalCountiesEl && totalCountiesEl.dataset.value) {
      totalCountiesEl.textContent = formatNumberResponsively(
        parseInt(totalCountiesEl.dataset.value)
      );
    }
  }, 250);
});
...
function updateCounters(data) {
  ...
  const totalParticipants =
    data.totalParticipants || liveStatsConfig.fallbackData.totalParticipants;
